...
\begin{frame}[plain]
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.20\textwidth}
      \hfill\includegraphics[width=0.78\linewidth]{assets/ufpi_brasao.png}
    \end{column}
  \end{columns}
  \vfill
...
\begin{frame}{Desenho experimental controlado}
  \vspace{-0.05cm}
  \includegraphics[height=3.45cm,width=\textwidth,keepaspectratio]{../paper/figures/methodology_pipeline.pdf}
  \vspace{0.05cm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      {\color{ufpidark}\bfseries Mesma base}\par
      1.050 treino, 225 validação e 225 teste.
    \end{column}
    \begin{column}{0.31\textwidth}
      {\color{ufpidark}\bfseries Mesmo detector}\par
      YOLO26n, entrada 640, lote 24, até 60 épocas.
    \end{column}
    \begin{column}{0.31\textwidth}
      {\color{ufpidark}\bfseries Três repetições}\par
...
% 7
\begin{frame}{Mesmas caixas, rótulos mais amplos}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.60\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot+[draw=ufpidark,fill=ufpidark] coordinates {(Fine,0.103) (Material,0.171) (Binary,0.466)};
        \end{axis}
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.37\textwidth}
      {\fontsize{19}{24}\selectfont\bfseries A geometria e a confiança permanecem fixas.}\par
      \vspace{0.35cm}
      O aumento ocorre ao aceitar rótulos menos específicos.
      \vspace{0.35cm}
      Isso evidencia uma penalização semântica importante, além das falhas de localização.
    \end{column}
  \end{columns}
\end{frame}

% 8
\begin{frame}{O que os intervalos de confiança sustentam}
  \vspace{0.1cm}
  {\fontsize{14.5}{18}\selectfont Bootstrap pareado por imagem, com 2.000 reamostragens em cada semente. Um resultado 3/3 indica IC de 95\% acima de zero nas três execuções.}

  \vspace{0.35cm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      \bigmetric{3/3}{Material melhora $F_1$ frente a Fine}
    \end{column}
    \begin{column}{0.31\textwidth}
      \bigmetric{1/3}{ganho de AP$_{50}$ de Material frente a Fine}
    \end{column}
    \begin{column}{0.31\textwidth}
      \bigmetric{3/3}{ganho hierárquico em AP$_{50}$}
    \end{column}
  \end{columns}

  \vspace{0.4cm}
  {\color{muted}\fontsize{12}{15}\selectfont Leitura prática: a melhora de Material é consistente no ponto operacional, mas sua superioridade global em AP permanece inconclusiva.}
\end{frame}
...
\begin{frame}{Os erros semânticos não explicam tudo}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.43\textwidth}
      \bigmetric{56,95\%}{dos casamentos espaciais recebem classe Fine incorreta}
      \vspace{0.75cm}
      \bigmetric{63,5\%}{dos 637 objetos de teste são omitidos}
    \end{column}
    \begin{column}{0.53\textwidth}
      {\fontsize{18}{23}\selectfont\bfseries Duas limitações coexistem}\par
      \vspace{0.28cm}
      \begin{itemize}
        \item confusão entre rótulos visualmente próximos
        \item cobertura espacial insuficiente, sobretudo por omissões
      \end{itemize}
      \vspace{0.25cm}
      {\color{muted}\fontsize{11.5}{14}\selectfont Média de três sementes, confiança 0,25 e IoU $\geq 0{,}5$ para casamentos espaciais.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Casos reais do teste Fine}
  ...
  \end{columns}
  \vspace{0.20cm}
  \centering{\color{muted}\fontsize{10.5}{12}\selectfont Predições da semente 42. Linhas brancas: verdade-terreno; linhas pretas tracejadas: predição.}
\end{frame}
